...
class Patch:
    ...
    def calc_stats(self):
        ...
        ca3  = pa.get_core_areas(patch, 3)
        ca6  = pa.get_core_areas(patch, 6)

        core_area1 = sum([len(i) for i in ca1])
        core_area3 = sum([len(i) for i in ca3])
        core_area6 = sum([len(i) for i in ca6])
        n_core_areas1 = len(ca1)
        n_core_areas3 = len(ca3)
        n_core_areas6 = len(ca6)
...
def sparseness(patch, archive):
    ...
    dists = scipy.spatial.distance.pdist([patch.stats] + [el.stats for el in archive], "seuclidean")
    ...
    dists = scipy.spatial.distance.pdist([patch.stats] + [el.stats for el in archive], "euclidean")
    dists = dists[:len(archive)]
    dists.sort()
